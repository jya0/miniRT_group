...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/28 21:16:38 by ol                #+#    #+#             */
/*   Updated: 2020/01/28 21:57:58 by pqueiroz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void
	do_loop_hook2(CFRunLoopTimerRef observer, void *info)
{
	(void)observer;
...
void
	do_loop_flush(CFRunLoopObserverRef observer, CFRunLoopActivity activity,
					void *info)
{
	mlx_ptr_t		*mlx_ptr;
	mlx_win_list_t	*win;
	...
	*mlx_init()
{
	...
	// super magic trick to detach app from terminal, get menubar & key
	// input events
	...
	{
		new_mlx->font->buffer[i + 0] = g_font_atlas.pixel_data[i + 2];
		new_mlx->font->buffer[i + 1] = g_font_atlas.pixel_data[i + 1];
		new_mlx->font->buffer[i + 2] = g_font_atlas.pixel_data[i + 0];
		((unsigned char *)new_mlx->font->buffer)[i + 3] =
			0xFF - g_font_atlas.pixel_data[i + 3];
		i += 4;
	}
	...
	mlx_loop(mlx_ptr_t *mlx_ptr)
{
	...
	observer = CFRunLoopObserverCreate(NULL, kCFRunLoopBeforeTimers, true, 0,
										do_loop_flush, &ocontext);
	CFRunLoopAddObserver(CFRunLoopGetMain(), observer, kCFRunLoopCommonModes);
	[NSApp run];
}

void
	mlx_pixel_put(mlx_ptr_t *mlx_ptr, mlx_win_list_t *win_ptr,
...
int
	mlx_loop_hook(mlx_ptr_t *mlx_ptr, void (*fct)(void *), void *param)
{
	...
	if (fct)
	{
		timer = CFRunLoopTimerCreate(kCFAllocatorDefault, 0.0, 0.0001, 0, 0,
										&do_loop_hook2, &tcontext);
